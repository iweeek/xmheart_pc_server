...
public interface XPWNavMapper {
	...
	@Select("SELECT n.id, n.column_id, n.child_column_id, c.column_name, c.column_name_en,\n" + 
			"n.article_title, n.brief, n.img_url, n.url \n" + 
			"FROM xpw_nav AS n \n" + 
			"LEFT JOIN xpw_column AS c \n" + 
			"ON n.child_column_id = c.id " + 
			" WHERE n.id = #{id, jdbcType = BIGINT}")
	@ResultMap(value = { "navWithColumnMapper" })
    XPWNav selectNavWithColumnByPrimaryKey(@Param("id") long id);
	...
	@Select("SELECT n.id, n.column_id, n.child_column_id, c.column_name, c.column_name_en,\n" + 
			...
			"LEFT JOIN xpw_column AS c \n" + 
			"ON n.child_column_id = c.id " + 
			" WHERE n.child_column_id = #{childColumnId, jdbcType = BIGINT}")
	@ResultMap(value = { "navWithChildColumnMapper" })
    List<XPWNav> selectNavWithColumnByChildColumnId(@Param("childColumnId") long childColumnId);
	...
	@ResultMap(value = { "navWithColumnMapper" })
    List<XPWNav> selectNavWithColumnByColumnId(@Param("columnId") long columnId);
}